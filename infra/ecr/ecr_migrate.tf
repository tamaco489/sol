resource "aws_ecr_repository" "sol_api_migrate" {
  ...
  tags = { Name = "${local.fqn}-api-migrate" }
}
...
resource "aws_ecr_lifecycle_policy" "sol_api_migrate" {
  repository = aws_ecr_repository.sol_api_migrate.name
